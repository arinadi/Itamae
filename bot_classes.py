...
class IdleMonitor:
    ...
    def __init__(self, app: Application, job_manager: "JobManager", shutdown_callback: Callable[[str], Any]):
        ...
        self.shutdown_on: Optional[float] = None
    ...
    def reset(self):
        """Force reset the idle timer when activity is detected."""
        if self.shutdown_on is not None or self.alerts_sent['first_alert']:
            log("IDLE", "Activity detected. Kitchen timer reset.")
    ...
    async def _handle_first_alert(self, remaining_minutes: float):
        if not self.job_manager.is_idle(): self.reset(); return
        ...
        try:
            ...
            self.alerts_sent['first_alert'] = True; log("IDLE", "First alert sent")
        except Exception as e: log("ERROR", f"Failed to send first alert: {e}")
    ...
    async def _handle_final_warning(self, remaining_minutes: float):
        ...
        try:
            ...
            self.alerts_sent['final_warning'] = True; log("IDLE", "Final warning sent")
        except Exception as e: log("ERROR", f"Telegram notification failed: {e}")
    ...
    async def _handle_shutdown(self):
        ...
        if self.shutdown_callback:
            if asyncio.iscoroutinefunction(self.shutdown_callback): await self.shutdown_callback("Automatic Idle Shutdown")
            else: self.shutdown_callback("Automatic Idle Shutdown")

    async def _monitor_loop(self):
        while True:
            await asyncio.sleep(60)
            if self.shutdown_imminent or not Config.ENABLE_IDLE_MONITOR or self.job_manager is None: continue
            try:
                if self.job_manager.is_idle():
                    ...
                    if elapsed_minutes >= Config.IDLE_FIRST_ALERT_MINUTES and not self.alerts_sent['first_alert']: await self._handle_first_alert(remaining_minutes)
                    if elapsed_minutes >= Config.IDLE_FINAL_WARNING_MINUTES and not self.alerts_sent['final_warning']: await self._handle_final_warning(remaining_minutes)
                    if remaining_minutes <= 0: await self._handle_shutdown()
                else: self.reset()
            except Exception as e: log("ERROR", f"IdleMonitor: {e}")
...
class JobManager:
    ...
    def __init__(self, app: Application, idle_monitor_ref: IdleMonitor, models_ready_event: asyncio.Event):
        ...
        self._sourcing_count = 0 # Track active metadata fetching/downloading
    ...
    def start_sourcing(self):
        self._sourcing_count += 1
        self.signal_activity()

    def stop_sourcing(self):
        self._sourcing_count = max(0, self._sourcing_count - 1)
        self.signal_activity()
    ...
    async def add_job(self, job: TranscriptionJob):
        ...
        content = (f"✅ *Queued:* `{job.video_title}`\n⏱️ Duration: `{duration_str}`\n🔢 Position: `#{queue_position}`{model_status_note}") if job.is_url_job else f"✅ *Queued:* `{job.original_filename}` (#{queue_position}){model_status_note}"
        keyboard = [[InlineKeyboardButton("❌ Cancel Order", callback_data=f"cancel_{job.job_id}")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        if job.thumbnail_url:
            try:
                await self.app.bot.send_photo(chat_id=job.chat_id, photo=job.thumbnail_url, caption=content, parse_mode=ParseMode.MARKDOWN, reply_to_message_id=job.message_id, reply_markup=reply_markup)
                log("JOB", f"[{job.job_id}] Queued (with thumbnail)"); return
            except Exception as e: log("ERROR", f"Failed to send thumbnail: {e}")
        await self.app.bot.send_message(chat_id=job.chat_id, text=content, parse_mode=ParseMode.MARKDOWN, reply_to_message_id=job.message_id, reply_markup=reply_markup)
        log("JOB", f"[{job.job_id}] Queued at #{queue_position}")

    def complete_job(self, job_id: str):
        if self.currently_processing and self.currently_processing.job_id == job_id: self.currently_processing = None
        if job_id in self.job_registry: del self.job_registry[job_id]
        log("JOB", f"[{job_id}] Completed"); self.signal_activity()

    def set_processing_job(self, job: TranscriptionJob):
        self.currently_processing = job; job.status = "processing"
        log("JOB", f"[{job.job_id}] Processing..."); self.signal_activity()

    async def cancel_job(self, job_id: str) -> tuple[bool, str]:
        job = self.job_registry.get(job_id)
        if not job: return False, "Unknown"
        job.status = "cancelled"; log("JOB", f"[{job.job_id}] Cancelled"); return True, job.original_filename
...
class FilesHandler:
    ...
    async def handle_files(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if attachment.file_size and attachment.file_size > (Config.BOT_FILESIZE_LIMIT * 1024 * 1024):
            ...
            await message.reply_text(f"❌ *File Too Large*\n\nThe file `{attachment.file_name}` ({file_size_mb:.2f} MB) exceeds the bot's download limit of *{Config.BOT_FILESIZE_LIMIT} MB*. Please send a smaller file.", parse_mode=ParseMode.MARKDOWN)
            return
        ...
        if multipart_match: await self._handle_multipart_chunk(message, file_obj, multipart_match)
            ...
            await file_obj.download_to_drive(local_path); await self._extract_and_queue_zip(local_path, original_filename, message)
        else:
            local_path = os.path.join(self.upload_folder, f"{uuid.uuid4().hex}_{secure_filename(original_filename)}")
            await file_obj.download_to_drive(local_path); await self._validate_and_queue_file(local_path, message)
    ...
    async def _handle_multipart_chunk(self, message: telegram.Message, file_obj: telegram.File, match: re.Match):
        ...
        loop = asyncio.get_running_loop(); archive_data = self.multipart_archives.get(base_name)
        if archive_data:
            archive_data['files'].append((local_path, original_filename))
            if archive_data.get('timer'): archive_data['timer'].cancel()
            ...
            status_message = await message.reply_text(f"ℹ️ Received the first part of archive `{base_name}`.\nSend the other parts. I will wait {self.COMBINE_TIMEOUT_SECONDS} seconds after the last file is received.", parse_mode=ParseMode.MARKDOWN)
            self.multipart_archives[base_name] = {'files': [(local_path, original_filename)], 'message': status_message, 'original_message': message, 'timer': loop.call_later(self.COMBINE_TIMEOUT_SECONDS, lambda: asyncio.create_task(self._process_multipart_archive(base_name)))}

    async def _process_multipart_archive(self, base_name: str):
        archive_data = self.multipart_archives.pop(base_name, None)
        if not archive_data: return
        status_message = archive_data['message']; file_tuples = archive_data['files']
        ...
        if len(file_tuples) < 1: await status_message.edit_text(f"❌ No files were available to combine for `{base_name}`."); return
    ...
    async def _extract_and_queue_zip(self, zip_path: str, zip_name: str, message: telegram.Message, status_message: telegram.Message = None):
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                if not file_list: await status_message.edit_text(f"⚠️ No valid files found inside `{zip_name}`."); return
            ...
            for root, _, files in os.walk(extract_dir):
                for filename in files:
                    ...
                    shutil.move(source_path, dest_path); await self._validate_and_queue_file(dest_path, message, filename_override=filename)
        ...
        except zipfile.BadZipFile: await status_message.edit_text(f"❌ Failed: `{zip_name}` is not a valid ZIP archive.", parse_mode=ParseMode.MARKDOWN)
        except Exception as e: await status_message.edit_text(f"❌ Error while extracting `{zip_name}`: `{e}`", parse_mode=ParseMode.MARKDOWN)
        finally:
